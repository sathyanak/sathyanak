...
PrimeFaces.widget.Ultima = PrimeFaces.widget.BaseWidget.extend({
    ...
    activate: function(item) {
        ...
        if(submenu.length) {
            ...
            item.addClass('active-menuitem-lastclicked');
        }
    },
    ...
    deactivate: function(item) {
        var submenu = item.children('ul');
        item.removeClass('active-menuitem');
        item.removeClass('active-menuitem-lastclicked');
    ...
    deactivateItems: function(items, animate) {
        ...
        for(var i = 0; i < items.length; i++) {
            ...
            else if(item.hasClass('active-menuitem') || item.hasClass('active-menuitem-lastclicked')) {
    ...
    addMenuitem: function (id) {
    	var found = $.inArray(id, this.expandedMenuitems) ; 
        if ( found > -1) {
        	var index = 0 ;
        	while ( (index = this.expandedMenuitems.indexOf(id)) > -1) {
    ...
    resetActiveMenuItem: function(menuId, menuItemId) {
		var items = $( '.active-menuitem-lastclicked' ) ;
		this.deactivateItems( items, false ) ;
		
		items = $( menuId ) ;
		this.addMenuitem( items[0].id ) ;

		items = $( menuItemId ) ;
		this.activate( items, false ) ;
		this.addMenuitem( items[0].id ) ;
    },
            ...
            for (var i = 0; i < this.expandedMenuitems.length; i++) {
                ...
                if (id) {
                    ...
                    if(submenu.length) {
                    	...
                    	lastMenuItemClicked = id ;
                    }
                }
            }
            $("#" + lastMenuItemClicked.replace(/:/g, "\\:")).addClass('active-menuitem-lastclicked');
        }
