...
PrimeFaces.widget.Barcelona = PrimeFaces.widget.BaseWidget.extend({
    ...
    init: function(cfg) {
        ...
        this.actions = this.wrapper.children('.sidebar-actions');
        this.actionMenu = this.actions.children('.action-menu');
        this.actionItems = this.actionMenu.children('li');
        this.actionLinks = this.actionItems.children('a');
        this.menuActive = false;
        this.sidebarClick = false;
        this.topbarLinkClick = false;
        this.topbarMenuClick = false;
        this.actionLinkClick = false;
    ...
    _bindEvents: function() {
        ...
        this.tabMenu.find('.menu-button').on('click', function(e) {
            ...
            $this.tabMenuNav.find(".active-item").removeClass("active-item");
        ...
        this.tabMenuNavLinks.on('click', function(e) {
            ...
            if ( $this.wrapper.hasClass('layout-wrapper-menu-active')						// Menu is open
            &&   $this.tabMenuNav.find(".active-item").index() == link.parent().index() )	// And it's this menu item
        	{																				// Close it.
            	link.parent().removeClass('active-item');
                $this.sidebar.css('overflow','visible');
                $this.wrapper.removeClass('layout-wrapper-menu-active');
                $this.tabMenuContents.find('.ripple-animate').remove();
            ...
            else
            {
	            ...
	            if ( $.cookie('barcelona_tabmenu_index') )
            	{
        ...
        this.actionLinks.on('click', function(e) {
            var link = $(this),
            item = link.parent(),
            submenu = link.next();

            $this.actionLinkClick = true;

            item.siblings('.active-sidemenuitem').removeClass('active-sidemenuitem');

            if($this.isDesktop()) {
                if(submenu.length) {
                    if(item.hasClass('active-sidemenuitem')) {
                        submenu.addClass('fadeOutDown');

                        setTimeout(function() {
                            item.removeClass('active-sidemenuitem'),
                            submenu.removeClass('fadeOutDown');
                        },500);
                    }
                    else {
                    	submenu.height( "auto" ) ;
                    	submenu.css( "max-height", "" + (link.offset().top - $(window).scrollTop()) + "px" ) ;
                        item.addClass('active-sidemenuitem');
                        submenu.addClass('fadeInUp');
                    }
                }
            }
            else {
            	submenu.height( "auto" ) ;
            	submenu.css( "max-height", "" + (link.offset().top - $(window).scrollTop()) + "px" ) ;
                item.children('ul').removeClass('fadeInUp fadeOutDown');
                item.toggleClass('active-sidemenuitem');
            }
        ...
        $(document.body).on('click', function() {
            if(!$this.topbarMenuClick && !$this.topbarLinkClick && !$this.actionLinkClick) {
                $this.topbarItems.filter('.active-topmenuitem').removeClass('active-topmenuitem');
                $this.actionItems.filter('.active-sidemenuitem').removeClass('active-sidemenuitem');
...
// Set Settle Shift Ccroll Height Resize
var settleShiftHeight = function () {
    var height = $(window).height() - $(".topbar").height() - $("#breadcrumb").height() - $("#settleShiftGroup").height() - $("#settleShiftMetrics").height();
    $('#settleShiftContent').height(height);
}

// Settle Shift height on resize
$(window).resize(function(){
    // Run Settle Shift Height
    settleShiftHeight();
})
...
$(document).ready(function () {
    ...
    $('.toggle-card').on('click', function () {
        $('.toggle-card .card-collapse').toggleClass('panel-toggle');
        $('.toggle-card .toggle-content').toggleClass('button-toggle');
    });

    // Toggle Bio
    $('.bio-toggle').on('click', function () {
        $('.bio-frame .card-collapse').toggleClass('panel-toggle');
        $('.bio-frame .bio-toggle .button').toggleClass('button-toggle');
    });

    // Close Modals
    $('.close-modal').on('click', function () {
        $('.ui-dialog.ui-widget.ui-widget-content').hide();
    ...
    // Settle Shift Select Associate Toggle
    $('.associate-select').on('click', function () {
        $(this).children('.ui-chkbox').children('.ui-chkbox-box').toggleClass('ui-state-active');
        $(this).children('.ui-chkbox').children('.ui-chkbox-box').children('.ui-chkbox-icon').toggleClass('ui-icon-blank');
        $(this).children('.ui-chkbox').children('.ui-chkbox-box').children('.ui-chkbox-icon').toggleClass('ui-icon-check');
    });
});
